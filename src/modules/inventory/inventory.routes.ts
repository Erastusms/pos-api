  ...
  properties: {
    ...
    meta: {
      ...
      properties: {
        ...
        hasNextPage: { type: 'boolean' }, hasPrevPage: { type: 'boolean' },
      },
    },
  },
})
...
export async function inventoryRoutes(app: FastifyInstance) {
  ...
  app.get('/', {
    preHandler: authRead,
    schema: {
      tags: tag, summary: 'Daftar inventory',
  ...
  app.get('/item/:id', {
    preHandler: authRead,
    schema: {
      tags: tag, summary: 'Detail inventory item by ID',
      security: [{ bearerAuth: [] }],
  ...
  app.get('/product/:productId', {
    preHandler: authRead,
    schema: {
      tags: tag, summary: 'Inventory berdasarkan product ID',
      security: [{ bearerAuth: [] }],
  ...
  app.post('/initial', {
    ...
    schema: {
      tags: tag, summary: 'Set stok awal produk',
      description: 'Hanya bisa dilakukan sekali per produk. Membuat InventoryItem + cost layer FIFO pertama.',
      security: [{ bearerAuth: [] }],
      body: {
        ...
        properties: {
          ...
          unit:        { type: 'string', default: 'pcs', description: 'Satuan: pcs, kg, liter, box, dll' },
          notes:       { type: 'string' },
  ...
  app.post('/adjust', {
    ...
    schema: {
      tags: tag, summary: 'Penyesuaian stok',
      description: [
      ...
      security: [{ bearerAuth: [] }],
      body: {
        type: 'object', required: ['inventoryItemId', 'type', 'quantity'],
        properties: {
          inventoryItemId: { type: 'string' },
          type: { type: 'string', enum: ['PURCHASE_IN', 'ADJUSTMENT_IN', 'RETURN_IN', 'ADJUSTMENT_OUT'] },
          quantity:    { type: 'number', minimum: 0.001 },
  ...
  app.get('/item/:id/history', {
    preHandler: authRead,
    schema: {
      tags: tag, summary: 'Riwayat pergerakan stok',
      ...
      querystring: {
        ...
        properties: {
          ...
          type: {
            type: 'string',
            enum: ['INITIAL', 'PURCHASE_IN', 'ADJUSTMENT_IN', 'ADJUSTMENT_OUT', 'SALE_OUT', 'RETURN_IN', 'VOID'],
          },
        },
      },
      response: {
  ...
  app.post('/item/:id/cogs-preview', {
    preHandler: authRead,
    schema: {
      tags: tag, summary: 'Preview COGS FIFO (read-only)',
